...
class WBVendorCodeExporter:
    def __init__(self):
        self.api_key = "eyJhbGciOiJFUzI1NiIsImtpZCI6IjIwMjUwNDE3djEiLCJ0eXAiOiJKV1QifQ.eyJlbnQiOjEsImV4cCI6MTc2MjIwMjA1OSwiaWQiOiIwMTk2OWY5NC1lY2M3LTc0NTgtYjI2ZC1kNmY1YzlkNGI0YjMiLCJpaWQiOjE4NDY4ODE5LCJvaWQiOjEyNzAzMzIsInMiOjc5MzQsInNpZCI6ImNjYjAyMjE5LWUxM2QtNDQ0Ni05OWQ0LWZiOWVlZWI1YmFkYSIsInQiOmZhbHNlLCJ1aWQiOjE4NDY4ODE5fQ.ccnEqrSvI76sVMzRcO4VsJq2WXZIcuPi3Nc4V2TJWhUtuw8-g-eVQtDpeREI78SRpAjlyBaIDCr7bn9gzQL8Vg"
    ...
    def get_cards_batch(self, cursor=None):
        ...
        payload = {
            "settings": {
                ...
                "filter": {"withPhoto": -1},
                "sort": {"column": "updateAt", "order": "desc"}  # Сортировка по updateAt
            }
        }
        for attempt in range(self.retry_attempts):
            try:
                ...
                data = response.json()
                print(f"Получено {len(data.get('cards', []))} артикулов")
                return data
    ...
    def process_batch(self, cursor):
        ...
        if len(cards) == self.max_limit:
            next_cursor = {
                "updatedAt": cards[-1]["updatedAt"],  # Исправлено на updatedAt
                "nmID": cards[-1]["nmID"],
        ...
        print(f"Выгружено: {len(self.vendor_codes)} | Скорость: {len(self.vendor_codes)/elapsed:.1f} арт/сек | Время блока: {block_time:.2f} сек", end='\r')
    ...
    def export_vendor_codes(self):
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            while cursors and not self.shutdown_requested:
                ...
                print(f"Обработка курсора: {cursor}")
                future = executor.submit(self.process_batch, cursor)
            ...
            # Повторная обработка пропущенных курсоров
            for attempt in range(2):
                ...
                current_missed = deque()
                while self.missed_cursors:
                    cursor = self.missed_cursors.popleft()
                    future = executor.submit(self.process_batch, cursor)
                    next_cursor = future.result()
                    if next_cursor:
                        cursor_key = json.dumps(next_cursor, sort_keys=True)
        ...
        if self.missed_cursors:
            print(f"\nОсталось {len(self.missed_cursors)} пропущенных курсоров")
